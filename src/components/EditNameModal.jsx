...
const EditNameModal = ({ isOpen, onClose, currentName, onSave }) => {
  ...
  const { stateDb, peerId } = useOrbit() || {};

  useEffect(() => {
    setName(currentName || localStorage.getItem('bloom_name') || '');
  }, [currentName, isOpen]);
  ...
  const handleSubmit = async (e) => {
    ...
    localStorage.setItem('bloom_name', cleanName);

    // 2. Synchronize to P2P network if connected to a room
    if (stateDb && peerId) {
      try {
        await stateDb.put(`peer_name_${peerId}`, cleanName);
      } catch (err) {
        console.warn('Failed to sync updated name to party:', err);
      }
    }

    // 3. Dispatch global events so UI updates reactively everywhere
    window.dispatchEvent(new CustomEvent('bloom:name-change', { detail: cleanName }));
    window.dispatchEvent(new CustomEvent('bloom:notify', { 
      detail: { text: `Name updated to "${cleanName}"`, type: 'system', sender: 'System' } 
    }));
      ...
      >
        ...
        <div className="mt-2 mb-8 text-center px-4">
          ...
          <p className="text-white/40 text-sm mt-2">
            Update your name across Bloom and connected parties.
          </p>
        </div>
