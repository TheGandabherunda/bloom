...
const Lobby = ({ onJoin, onCreateRoom }) => {
  ...
  const filteredRooms = Object.values(rooms)
    .filter(r => r.roomId.toLowerCase().includes(searchQuery.toLowerCase()) || (r.hostName && r.hostName.toLowerCase().includes(searchQuery.toLowerCase())))
  ...
  return (
    <div className="h-[100dvh] w-screen overflow-hidden flex flex-col antialiased bg-black relative">
      ...
      {/* Background */}
      <div className="absolute inset-0 items-center justify-center pointer-events-none z-0 flex">
        <img src="./assets/Bloom.svg" className="w-[150vw] sm:w-[90vw] max-w-[800px] opacity-[0.05] rotate-slow" alt="" />
      </div>
      ...
      <header className="bg-black/40 backdrop-blur-xl p-4 shadow-sm flex flex-col md:flex-row items-center justify-between border-b border-white/10 shrink-0 z-40 relative gap-4">
        <div className="flex items-center gap-3 w-full md:w-auto justify-between">
          <h2 className="font-bold text-white tracking-wide text-2xl">Bloom Lobby</h2>
          <button 
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-8 z-10">
        <div className="max-w-7xl mx-auto space-y-6">
          <h3 className="text-sm font-bold text-white/40 uppercase tracking-widest flex items-center gap-2">
            ...
            Live Global Rooms ({filteredRooms.length})
          </h3>
          ...
          {filteredRooms.length === 0 ? (
            <div className="text-center py-20 text-white/30">
              ...
              <p className="text-sm mt-2">Create one to start broadcasting!</p>
            </div>
          ) : (
              ...
              {filteredRooms.map(room => (
                ...
                >
                  <div className="flex justify-between items-start">
                    <div>
                      <h4 className="text-white font-bold text-lg">{room.roomId}</h4>
