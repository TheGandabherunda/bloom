...
function loadVideoInternal(videoObj, startSeconds = 0, targetState = 1) {
    ...
    // Build HD cover: prefer maxresdefault, fallback through sddefault → hqdefault → original thumbnail
    const rawCover = Array.isArray(videoObj.thumbnail) ? videoObj.thumbnail[0] : videoObj.thumbnail;
    const hdFallbacks = videoObj.id ? [
        `https://img.youtube.com/vi/${videoObj.id}/maxresdefault.jpg`,
        `https://img.youtube.com/vi/${videoObj.id}/sddefault.jpg`,
        `https://img.youtube.com/vi/${videoObj.id}/hqdefault.jpg`,
        rawCover
    ].filter(Boolean) : [rawCover].filter(Boolean);

    function loadHDCover(urls, index = 0) {
        ...
        if (!url || url === 'undefined') { loadHDCover(urls, index + 1); return; }
        const testImg = new Image();
        testImg.onload = () => {
            // maxresdefault returns a 120x90 placeholder when unavailable — skip it
            if (testImg.naturalWidth <= 120 && index === 0) { loadHDCover(urls, index + 1); return; }
            ui.audioThumbnail.src = url; ui.audioThumbnail.style.display = 'block'; ui.audioPlaceholder.style.display = 'none';
            ui.nowPlayingCover.src = url; ui.nowPlayingCover.classList.remove('hidden');
            updateAmbientColors(url);
        };
        testImg.onerror = () => loadHDCover(urls, index + 1);
        testImg.src = url;
    }

    const finalCover = hdFallbacks[0];
    if (finalCover) {
        loadHDCover(hdFallbacks);
    } else {
        ui.audioThumbnail.style.display = 'none'; ui.audioPlaceholder.style.display = 'flex';
        ui.nowPlayingCover.classList.add('hidden'); updateAmbientColors(null);
    }
...
(function initVisualizer() {
    // Inject canvas below cover art inside the audio-cover inner container
    ...
    function ensureCanvas() {
        const cover = ui.audioCover;
        if (!cover) return;
        const innerDiv = cover.querySelector('div.relative');
        const target = innerDiv || cover;
        if (!cover.contains(canvas)) target.appendChild(canvas);
    }

    function resizeCanvas() {
        const w = Math.min((canvas.parentElement && canvas.parentElement.offsetWidth) || 460, 460);
        canvas.width = w;
        canvas.height = 72;
    }
    ...
    async function startVisualizer() {
        ...
        // Connect YouTube iframe audio via MediaElementSource
        if (!source) {
            try {
                const iframe = document.getElementById('player');
                const iframeWindow = iframe?.contentWindow;
                // We use a silent oscillator as heartbeat + the visualizer reacts to
                // simulated frequency data driven by the player state instead,
                // since cross-origin iframe audio capture is blocked by browsers.
                // We drive a convincing fake analyser with real timing data.
            } catch (e) {}
        }
    ...
    // Since YouTube audio is cross-origin we can't tap the MediaStream.
    // Instead we build a convincing procedural visualizer synced to playback state.
    const NUM_BARS = 48;
    const barPhases = Array.from({ length: NUM_BARS }, (_, i) => Math.random() * Math.PI * 2);
    const barSpeeds = Array.from({ length: NUM_BARS }, (_, i) => 0.018 + Math.random() * 0.022);
    const barAmps   = Array.from({ length: NUM_BARS }, (_, i) => {
        // Shape: higher in middle-low freq range (bars 4-18), lower at extremes
        const norm = i / NUM_BARS;
        const peak = Math.exp(-Math.pow((norm - 0.28) * 3.5, 2)) * 0.9
                   + Math.exp(-Math.pow((norm - 0.55) * 4, 2)) * 0.55
                   + 0.08;
        return peak;
    });
    ...
    function drawBars(timestamp = 0) {
        ...
        const playFactor = isPlaying ? 1 : 0;

        // Advance phases only when playing
        if (isPlaying) {
            for (let i = 0; i < NUM_BARS; i++) barPhases[i] += barSpeeds[i] * dt;
        }

        // Dynamic global amplitude: pulse on beat-ish interval
        const beatPulse = isPlaying ? (0.75 + 0.25 * Math.sin(timestamp * 0.0028)) : 0;
        const globalAmp = beatPulse * playFactor;

        const barW = W / (NUM_BARS * 1.6);
        const gap = barW * 0.6;
        const totalW = NUM_BARS * (barW + gap) - gap;
        const startX = (W - totalW) / 2;

        // Gradient colours from CSS vars (ambient colours)
        const c1 = getComputedStyle(document.documentElement).getPropertyValue('--color-1').trim() || '#be0aff';
        const c2 = getComputedStyle(document.documentElement).getPropertyValue('--color-2').trim() || '#ff8700';
        const grad = ctx.createLinearGradient(0, H, 0, 0);
        grad.addColorStop(0, c1 || '#be0aff');
        grad.addColorStop(1, c2 || '#0aefff');

        for (let i = 0; i < NUM_BARS; i++) {
            const raw = Math.abs(Math.sin(barPhases[i]) * 0.6 + Math.sin(barPhases[i] * 1.7 + 0.5) * 0.4);
            const h = Math.max(3, barAmps[i] * raw * globalAmp * H * 0.72);
    ...
    // Hook into setPlayerMode / play state changes
    function updateVisualizerState() {
        const inAudioMode = currentPlayerMode === 'audio' && !isLyricsMode;
        if (inAudioMode) {
            ensureCanvas();
    ...
    // Patch onPlayerStateChange to trigger visualizer
    const _origStateChange = window.onPlayerStateChange || function(){};
    const origOnPlayerStateChange = onPlayerStateChange;
    const patchedOnPlayerStateChange = function(event) {
    ...
    window.addEventListener('load', () => {
        if (ytPlayer && ytPlayer.addEventListener) {
            ytPlayer.addEventListener('onStateChange', updateVisualizerState);
        }
    });

    // Observe audio mode class on videoWrapper to start/stop
    const observer = new MutationObserver(() => updateVisualizerState());
    const vw = ui.videoWrapper;
    if (vw) observer.observe(vw, { attributes: true, attributeFilter: ['class'] });

    window.startMusicVisualizer = startVisualizer;
    window.stopMusicVisualizer = stopVisualizer;
    window.updateVisualizerState = updateVisualizerState;
})();