    ...
    AppState.members.forEach(m => {
        ...
        if (!isSelf) {
            if (AppState.myRole === ROLES.OWNER) {
                actionsHtml = `<div class="flex items-center gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                    ${isAdmin ? `<button class="p-2 bg-slate-800 hover:bg-slate-700 rounded-full text-slate-400 hover:text-yellow-400 transition-colors flex items-center justify-center shadow-sm" title="Demote to Member" onclick="executeCommand('SET_ROLE', { targetId: '${m.id}', role: '${ROLES.MEMBER}' })"><span class="material-symbols-rounded text-[18px]">gpp_bad</span></button>` : `<button class="p-2 bg-slate-800 hover:bg-slate-700 rounded-full text-slate-400 hover:text-yellow-400 transition-colors flex items-center justify-center shadow-sm" title="Promote to Admin" onclick="executeCommand('SET_ROLE', { targetId: '${m.id}', role: '${ROLES.ADMIN}' })"><span class="material-symbols-rounded text-[18px]">gpp_good</span></button>`}
                    <button class="p-2 bg-slate-800 hover:bg-slate-700 rounded-full text-slate-400 hover:text-red-400 transition-colors flex items-center justify-center shadow-sm" title="Kick User" onclick="executeCommand('KICK', { targetId: '${m.id}' })"><span class="material-symbols-rounded text-[18px]">person_remove</span></button></div>`;
            } else if (AppState.myRole === ROLES.ADMIN && m.role === ROLES.MEMBER) {
                actionsHtml = `<div class="flex items-center gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity"><button class="p-2 bg-slate-800 hover:bg-slate-700 rounded-full text-slate-400 hover:text-red-400 transition-colors flex items-center justify-center shadow-sm" title="Kick User" onclick="executeCommand('KICK', { targetId: '${m.id}' })"><span class="material-symbols-rounded text-[18px]">person_remove</span></button></div>`;
            }
        }
